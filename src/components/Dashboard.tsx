...
interface DashboardProps {
  ...
  onCreateProjectFromTemplate: (templateId: string, projectData: {
    title: string;
    description?: string;
    startDate: Date;
    deadline: Date;
...
function Dashboard({
  ...
  const stats = [{
    ...
    subtitle: <div className="flex flex-col items-center gap-1">
          ...
          {upcomingEvents.length > 0 && <div className="text-xs text-gray-400">
  ...
  const formatTaskTime = (task: Task) => {
    ...
    if (isToday(task.scheduledStart)) {
      ...
      return format(task.scheduledStart, 'EEEE à HH:mm', {
        locale: fr
      });
    } else {
      return format(task.scheduledStart, 'dd MMM à HH:mm', {
        locale: fr
      });
    }
  };
  const formatEventTime = (event: Event) => {
    if (isToday(event.startDate)) {
      ...
      return format(event.startDate, 'EEEE à HH:mm', {
        locale: fr
      });
    } else {
      return format(event.startDate, 'dd MMM à HH:mm', {
        locale: fr
      });
    }
  };
  ...
  return <div className="max-w-7xl mx-auto space-y-6 sm:space-y-8 animate-fade-in px-2 sm:px-4 lg:px-0">
      {/* En-tête */}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
        ...
        return <div key={index} className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-4 lg:p-6 shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
              <div className="flex flex-col items-center text-center space-y-2 sm:space-y-3 lg:space-y-4">
                ...
                <div>
                  <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-1">
                    {stat.value}
                    {stat.total && <span className="text-sm sm:text-base lg:text-lg text-gray-500 font-normal">
                        /{stat.total}
                      </span>}
                  </div>
                ...
                {stat.total && <div className="w-full">
                    <div className="bg-gray-200 rounded-full h-1.5 sm:h-2">
                      <div className={`h-1.5 sm:h-2 rounded-full transition-all duration-500 bg-gradient-to-r ${classes}`} style={{
      ...
      {/* Tâches en retard */}
      {overdueTasks.length > 0 && <div className="bg-gradient-to-r from-red-50 to-orange-50 border-2 border-red-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg">
          ...
          return <div key={task.id} className="flex items-center justify-between bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl shadow-sm border border-red-100 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    {onUpdateTask && <button type="button" onClick={e => handleTaskCompletion(task, e)} className="p-1.5 sm:p-2 hover:bg-green-100 rounded-lg transition-colors flex-shrink-0" title="Marquer comme terminé">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {projectName && <div className="flex items-center gap-1 flex-shrink-0">
                      ...
                      <div className="text-xs sm:text-sm text-red-600 font-medium">
                        {task.scheduledStart && task.scheduledStart > new Date(task.deadline) ? `Planifiée après l'échéance: ${format(task.scheduledStart, 'dd/MM/yyyy à HH:mm')}` : `Échéance dépassée: ${format(task.deadline, 'dd/MM/yyyy à HH:mm')}`}
                      </div>
                    </div>
                  </div>
                  {onUpdateTask && <button type="button" onClick={() => handleTaskClick(task)} className="p-1.5 sm:p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-md transition-colors flex-shrink-0" title="Modifier la tâche">
                      <Edit3 size={14} />
                    </button>}
                </div>;
        })}
            {overdueTasks.length > 3 && <p className="text-xs sm:text-sm text-red-600 text-center font-medium bg-white p-2 rounded-lg">
      ...
      {todayEvents.length > 0 && <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-lg">
          <div className="flex items-center gap-3 mb-4 sm:mb-6">
          ...
          <div className="space-y-2 sm:space-y-3">
            {todayEvents.map(event => <div key={event.id} className="flex items-center justify-between bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl shadow-sm group hover:shadow-md transition-shadow">
                <div className="flex-1 min-w-0">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="p-4 sm:p-6">
            {getUpcomingEvents().length > 0 ? <div className="space-y-2 sm:space-y-3">
                {getUpcomingEvents().map(event => <div key={event.id} className="flex items-center justify-between p-3 sm:p-4 bg-purple-50 rounded-lg sm:rounded-xl hover:bg-purple-100 transition-colors group">
                    <div className="flex-1 min-w-0">
                      ...
                      {event.location && <p className="text-xs sm:text-sm text-gray-600 mt-1 truncate">📍 {event.location}</p>}
                    </div>
                    <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                      {event.allDay ? <span className="px-2 sm:px-3 py-1 rounded-full text-xs font-medium bg-purple-200 text-purple-800">
                          Toute la journée
                        </span> : <span className="text-xs sm:text-sm text-gray-500 font-medium hidden sm:inline">
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-sm border border-gray-100">
          ...
          <div className="p-4 sm:p-6">
            {getUpcomingTasks().length > 0 ? <div className="space-y-2 sm:space-y-3">
                {getUpcomingTasks().map(task => {
              const projectName = getProjectName(task.projectId);
              return <div key={task.id} className="flex items-center justify-between p-3 sm:p-4 bg-gray-50 rounded-lg sm:rounded-xl hover:bg-gray-100 transition-colors group cursor-pointer" onClick={() => handleTaskClick(task)}>
                      <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                        {onUpdateTask && <button type="button" onClick={e => handleTaskCompletion(task, e)} className="p-1 hover:bg-white rounded flex-shrink-0" title="Marquer comme terminé">
                      ...
                      <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
                        <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium ${task.priority === 'urgent' ? 'bg-red-100 text-red-800' : task.priority === 'high' ? 'bg-orange-100 text-orange-800' : task.priority === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>
                        ...
                        {onUpdateTask && <Edit3 className="text-gray-400 group-hover:text-blue-600 transition-colors hidden sm:inline" size={14} />}
                      </div>
                    </div>;
            })}
              </div> : <div className="text-center py-6 sm:py-8">
